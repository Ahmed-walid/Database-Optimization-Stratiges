...
def generateEmployeeData():
    ...
    for i in range(1,numOfEmployees +1):
        ...
        employeeData.append([i, "FIRST_NAME" + str(i), "LAST_NAME" + str(i), random.randint(1, numOfDeptartments), random.randint(5000 , 50001) ,random.choice(['M','F']), generatePhoneNumber(), random.randint(1, numOfBranchs)])
    return employeeData
...
def generateCompliantsData():
    ...
    for i in range(1, numOfComplaints + 1):
        ...
        complaintsData.append([i,random.choice([0,1]), 'COMPLAINT' + str(i), generateRandomDate(), random.randint(1, numOfCustomers), random.randint(1, numOfEmployees)])
...
def generateOfferData():
    ...
    for i in range(1, numOfOffers + 1):
        ...
        offerData.append([i, random.randint(min_offer_price, max_offer_price), random.randint(min_offer_minutes, max_offer_minutes), random.randint(min_offer_megas, max_offer_megas), generateRandomDate()])
    return offerData
...
csv_path = '../../non-optimized-csvs'

# main function
def main():
    employeeData = generateEmployeeData()
    writeCSV(employeeData, csv_path+"/employee.csv")
    customerData = generateCustomerData()
    writeCSV(customerData, csv_path+"/customer.csv")
    departmentData = generateDepartmentData()
    writeCSV(departmentData, csv_path+"/department.csv")
    cardData = generateCardData()
    writeCSV(cardData, csv_path+"/card.csv")
    serviceData = generateServiceData()
    writeCSV(serviceData, csv_path+"/service.csv")
    complaintsData = generateCompliantsData()
    writeCSV(complaintsData, csv_path+"/complaints.csv")
    branchData = generateBranchData()
    writeCSV(branchData, csv_path+"/branch.csv")
    offerData = generateOfferData()
    writeCSV(offerData, csv_path+"/offer.csv")
    faqData = generateFAQData()
    writeCSV(faqData, csv_path+"/faq.csv")
    planData = generatePlanData()
    writeCSV(planData, csv_path+"/plan.csv")
    customer_offerData = generateCustomer_OfferData()
    writeCSV(customer_offerData, csv_path+"/customer_offer.csv")
    customer_serviceData = generateCustomer_ServiceData()
    writeCSV(customer_serviceData, csv_path+"/customer_service.csv")
